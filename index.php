...
if (isset($_POST['action']) && $_POST['action'] == 'run_crawler') {
    ...
    $script_path = "C:\\Users\\Gwen\\anaconda3\\twse\\twse_sync.py"; 

    if (empty($c_start) || empty($c_end)) {
        $error = "請選擇正確的日期區間。";
    } else {
        // 2. 組合指令 (Windows 環境專用穩定寫法)
        // 注意：外層要有一對雙引號包住整個指令，內層路徑也要有雙引號
        $cmd = "cmd /c \"\"$python_path\" \"$script_path\" --start $c_start --end $c_end\" 2>&1";
        ...
        // 4. 判斷是否成功
        if ($result !== null && strpos($result, '批次匯入完成') !== false) {
...
<form method="post" action="index.php"> <input type="hidden" name="action" value="run_crawler"> <div class="col-md-4">
					<label class="form-label">同步起點</label>
					<input type="date" name="crawl_start" class="form-control" value="2025-11-01">
				</div>
				<div class="col-md-4">
					<label class="form-label">同步終點</label>
					<input type="date" name="crawl_end" class="form-control" value="2025-11-30">
				</div>
				<div class="col-md-4">
						<button type="submit" class="btn btn-primary w-100">啟動遠端 Python 同步</button>
				</div>
			</form>
